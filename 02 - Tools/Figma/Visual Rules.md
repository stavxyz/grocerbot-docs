...
15. **The registry has no treatment.** Built 2026-09-07 on the SYSTEM REGISTRY page from the example registry's shape: a charter sheet; six bands (Principles, Actors, Interfaces, Artifacts, Piping, Data), each a list at the left and one specimen per entry at the right. A list row is KIND (a Color Tag: grey for actors and interfaces, blue for artifacts, brown for piping, pink for data), NAME, DESCRIPTION, HOST (a Tools mark), and WHERE (the boards, as `P1 · P3`). Three things need a ruling: that this is the treatment; whether operations get a band; and whether a registry pass ends every part's run, which the Function doc does not yet say.
