...
- **backstop**: the two scheduled jobs that catch a failure the wrapper missed, the timeout reaper and the DLQ monitor. Settled in code (`workflows/backstops/`) and in the framework spec ("three-mechanism failure detection").
- **phantom**: a `job_enqueues` row older than thirty minutes with no `job_executions` row: a push that happened and a handler that never ran. Settled in code (`phantom_reconciliation.py`); the ops summary's footer counts them.
- **kernel task**: a scheduled job about the orchestrator itself rather than a vendor, seeded under the `system` vendor: the ops summary, the dedup gauges, the phantom count, and the two backstops. The observability doc's word; the code has the `system.*` job type prefix and no noun.
...
| 9 | Registry build | Two tables named `vendors`, one in the orchestrator's database (the seed-reconciled vendor registry) and one in core's (four seeded rows with mappings) | vendors (orchestrator); vendors (core) | One name, two tables in two databases; no board holds both, so no board says so. The registry uses the parenthesised forms. | Part 1 and Part 4 boards; the registry's Data band |
